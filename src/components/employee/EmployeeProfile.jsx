...
export function EmployeeProfile() {
  ...
  useEffect(() => {
    ...
    client.connect({}, () => {
      client.subscribe("/topic/user/profile", (message) => {
      ...
      if (response.status === 200) {
        ...
        if (result) {
          ...
          axios
            ...
            .put(
              process.env.REACT_APP_API_EMPLOYEES_PHOTO + "/" + idEmployee,
              photoEmployee
            )
            .then((response) => {
    ...
    if (e === "Đang chờ") {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axios
            .put(
              process.env.REACT_APP_API_EMPLOYEES_STATUS_BAN + "/" + idEmployee
            )
            .then((res) => {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axios
            .put(
              process.env.REACT_APP_API_EMPLOYEES_STATUS_ACTIVE +
                "/" +
                idEmployee
            )
            .then((res) => {
      ...
      >
        ...
        >
          <div className="d-flex">
            <TipsAndUpdatesIcon />
            <span>
              Đây là hồ sơ hiện tại của bạn. Có thể chỉnh sửa để thu hút thêm
              khách hàng.
            </span>
          </div>
        </div>
