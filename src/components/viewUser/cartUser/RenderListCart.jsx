...
export function RenderListCart({
  ...
  const handleDeletedCart = (value) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${process.env.REACT_APP_API_CARTS}/${value}`)
          .then((res) => {
  ...
  const handleUpdateContract = (id) => {
    ...
    if (
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axios
            .put(`${process.env.REACT_APP_API_CARTS_STATUS}/${findById.id}`)
            .then((res) => {
              Swal.fire({
                title:
                  "Chuyển tiền vào số tài khoản này để gặp hộ lý, số tiền là 200.000 VNĐ",
                html:
  ...
  const handleUpdate = (value) => {
    ...
    const checkMember = memberOfFamilyList.find(
      (e) => e.id === findById?.memberOfFamily
    );
  ...
  useEffect(() => {
    ...
    client.connect({}, () => {
      client.subscribe("/topic/saler", (message) => {
        ...
        toast.success("Yêu cầu tạo hợp đồng của bạn đã thành công");
      });
    });
  ...
  return (
    <>
      {listCart?.map((e) => {
        const mergedData = e.historyWorkingResponseList.reduce(
          (result, currentItem) => {
            ...
            if (existingItem) {
              if (
                !existingItem.sessionOfDate.includes(
                  currentItem.sessionOfDateName
                )
              ) {
