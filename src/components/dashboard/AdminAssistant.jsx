  ...
  useEffect(() => {
    const axiosData = async () => {
      const assistant = await axios.get(
        process.env.REACT_APP_API_EMPLOYEES_WAITING
      );
      const data = assistant.data.content;
      const listWithIndex = data.map((item, index) => ({
        ...item,
        index: index + 1,
      }));
  ...
  useEffect(() => {
    ...
    client.connect({}, () => {
      client.subscribe("/topic/admin", (message) => {
        ...
        toast.success("Có yêu cầu duyệt nhân viên mới");
      });
    });
